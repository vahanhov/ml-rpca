...
while logData.frob_err(t) / normM >= opts.tolerance && t<opts.maxIter % convergence check
    ...
    if false %isfield(mlsvd_opts, 'coarseThreshold') && frob_err(t-1)/normM < mlsvd_opts.coarseThreshold 
        disp('--Doing fine low rank projection--');
        ...
        [L_t, U_t, Sig_t, V_t] = mlRankApprox( M-S_t, @lansvd,...
            {r_hat+1, 'L'}, opts );
    end
